...
import com.kdde.basemodule.basemodule.dao.ExternalExperimentADao;
import com.kdde.basemodule.basemodule.entity.ExternalExperimentAEntity;
import com.kdde.basemodule.basemodule.service.ExternalExperimentAService;


@Service("externalExperimentAService")
public class ExternalExperimentAServiceImpl extends ServiceImpl<ExternalExperimentADao, ExternalExperimentAEntity> implements ExternalExperimentAService {

    @Override
    public PageUtils queryPage(Map<String, Object> params) {
        IPage<ExternalExperimentAEntity> page = this.page(
                new Query<ExternalExperimentAEntity>().getPage(params),
                new QueryWrapper<ExternalExperimentAEntity>()
        );
    ...
    @Override
    public List<ExternalExperimentAEntity> searchAnyCol(String keyword, List<String> colName) {
        QueryWrapper<ExternalExperimentAEntity> queryWrapper = new QueryWrapper<>();
        if(colName != null && !colName.isEmpty()){
            for (String column : colName) {
                queryWrapper.or().like(column, keyword);
            }
        }else{
            List<String> colNames = EntityColumnUtil.getTableColumns(ExternalExperimentAEntity.class);
