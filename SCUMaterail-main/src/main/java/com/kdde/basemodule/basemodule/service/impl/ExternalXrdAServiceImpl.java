...
import com.kdde.basemodule.basemodule.dao.ExternalXrdADao;
import com.kdde.basemodule.basemodule.entity.ExternalXrdAEntity;
import com.kdde.basemodule.basemodule.service.ExternalXrdAService;


@Service("externalXrdAService")
public class ExternalXrdAServiceImpl extends ServiceImpl<ExternalXrdADao, ExternalXrdAEntity> implements ExternalXrdAService {

    @Override
    public PageUtils queryPage(Map<String, Object> params) {
        IPage<ExternalXrdAEntity> page = this.page(
                new Query<ExternalXrdAEntity>().getPage(params),
                new QueryWrapper<ExternalXrdAEntity>()
        );
    ...
    @Override
    public List<ExternalXrdAEntity> searchAnyCol(String keyword, List<String> colName) {
        QueryWrapper<ExternalXrdAEntity> queryWrapper = new QueryWrapper<>();
        if(colName != null && !colName.isEmpty()){
            for (String column : colName) {
                queryWrapper.or().like(column, keyword);
            }
        }else{
            List<String> colNames = EntityColumnUtil.getTableColumns(ExternalXrdAEntity.class);
