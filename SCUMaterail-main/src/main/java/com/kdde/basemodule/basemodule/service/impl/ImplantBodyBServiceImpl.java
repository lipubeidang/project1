...
@Service("implantBodyBService")
public class ImplantBodyBServiceImpl extends ServiceImpl<ImplantBodyBDao, ImplantBodyBEntity> implements ImplantBodyBService {


    @Autowired
    ImplantBodyBDao implantBodyBDao;


    @Override
    public PageUtils queryPage(Map<String, Object> params) {
        IPage<ImplantBodyBEntity> page = this.page(
                new Query<ImplantBodyBEntity>().getPage(params),
                new QueryWrapper<ImplantBodyBEntity>()
        );
    ...
    @Override
    public Integer sum() {
        QueryWrapper<ImplantBodyBEntity> wrapper = new QueryWrapper<>();
        return implantBodyBDao.selectCount(wrapper);
    }
    ...
    @Override
    public List<ImplantBodyBEntity> searchAnyCol(String keyword, List<String> colName) {
        QueryWrapper<ImplantBodyBEntity> queryWrapper = new QueryWrapper<>();
        if(colName != null && !colName.isEmpty()){
            for (String column : colName) {
                queryWrapper.or().like(column, keyword);
            }
        }else{
            List<String> colNames = EntityColumnUtil.getTableColumns(ImplantBodyBEntity.class);
