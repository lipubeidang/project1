      ...
      <main class="main-content">
        ...
        <div class="page-banner animate-slide-up">
          ...
          <div class="banner-content">
            ...
            <div class="banner-info">
              <h1 class="page-title">模版数据</h1>
              <p class="page-desc" v-if="selectedTemplate">
                当前模版：{{ selectedTemplate.name }}
              </p>
              <p class="page-desc" v-else-if="currentCategory">
                当前目录：{{ currentCategory.name }} - 请选择模版
              </p>
              <p class="page-desc" v-else>查看已审核通过的模版数据</p>
            </div>
        ...
        <div class="steps-nav animate-slide-up delay-1">
          ...
          <div class="step" :class="{ active: step >= 3 }">
            ...
            <span class="step-label">查看数据</span>
          </div>
        </div>
        ...
        <div class="template-section animate-slide-up delay-2" v-if="step === 2" v-loading="loadingTemplates">
          <div class="section-header">
            <h3><el-icon><Files /></el-icon> 选择模版</h3>
          ...
          <el-empty v-else description="该分类下暂无模版" :image-size="120" />
        </div>

        <!-- 数据展示区域 (step 3) -->
        <div class="data-section animate-slide-up delay-2" v-if="step === 3" v-loading="loadingData">
          <div class="section-header">
            <h3><el-icon><DataBoard /></el-icon> 模版数据</h3>
            <div class="header-actions">
              <el-tag type="success" size="large">共 {{ templateData.length }} 条数据</el-tag>
              <el-button text @click="goBackToTemplates">
                <el-icon><ArrowLeft /></el-icon> 返回选择模版
              </el-button>
            </div>
          </div>

          <!-- 数据卡片列表 -->
          <div class="data-cards" v-if="templateData.length > 0">
            <div 
            ...
            >
              <div class="card-header">
                <div class="card-id">
                  ...
                  <el-tag type="success" effect="light">已审核</el-tag>
                </div>
              </div>
        ...
        <div class="welcome-section animate-slide-up delay-2" v-if="step === 1">
          <div class="welcome-card">
            ...
            <h2>请从左侧选择模版分类</h2>
            <p>选择分类后，将展示该分类下的所有模版，然后您可以选择模版查看已审核通过的数据</p>
          </div>
        </div>
      </main>
    </div>
...
import { 
  Folder, Document, User, DataLine, DataBoard,
  Files, ArrowLeft, Picture, Download, Pointer,
  ZoomIn, View, Loading, CircleCheck
...
const loadTemplateData = async () => {
  ...
  try {
    // 直接调用获取已审核数据的接口，而不是获取所有数据再过滤
    const response = await request.get(`/templateData/getAuditedTemplateData/${selectedTemplate.value.id}`)
    if (response.data?.code === 1) {
...
const previewFile = async (fileId, fieldName) => {
  ...
  try {
    ...
    if (isTifFile(fieldName) || blob.type === 'image/tiff' || blob.type === 'image/tif') {
      previewType.value = 'image'
